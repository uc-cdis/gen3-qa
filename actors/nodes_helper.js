...
class Node {
  ...
  getFieldOfType(field_type) {
    ...
    return Object.keys(this.data).find((key) => { return typeof this.data[key] === field_type })
  }
}
...
const _getAllNodes = function() {
  ...
  try {
    let nodes = JSON.parse(fs.readFileSync(_getDataPathString('NodeDescriptions.json')));
    let nodes_dict = {};
...
const _pathWithFileNode = function(all_nodes) {
  ...
  let all_nodes_clone = _cloneNodes(all_nodes);
  let file_node_name = Object.keys(all_nodes_clone).find((node_name) => { return all_nodes_clone[node_name].category === 'data_file' });
  let nodes_path = _nodePathToProject(file_node_name, all_nodes_clone);
  let file = all_nodes_clone[file_node_name].clone();
...
module.exports = {
  ...
  ithNodeInPath(i) {
    ...
    let nodes_list = Object.values(path_and_file.path);
