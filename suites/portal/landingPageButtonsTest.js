...
Scenario('Navigate to the landing page and click on buttons @landing', async ({ I, home, users }) => {
  ...
  for (const [key, button] of Object.entries(buttons)) {
    ...
    btExists = await tryTo(() => I.seeElement({"xpath": `//button[contains(text(), \'${button.text}\')]`}) );
    if (btExists) {
      assertionFailureMsg = `button ${button.text} did not redirect user to expected url ${button.expectedUrl}.`;
      I.click({"xpath": `//button[contains(text(), \'${button.text}\')]`});
      await sleepMS(500);
      theUrl = await I.grabCurrentUrl();
      expect(
        theUrl,
        assertionFailureMsg,
