...
Scenario('The query button and data card work for the user with query access', ifInteractive(
  async (I) => {
...
Scenario('The query button and data card are not visible for the user without query access', ifInteractive(
  async (I) => {
    const result = await interactive(`
...
Scenario('On the projects page, the submit button is only present for projects the user may submit to - otherwise only browse is present', ifInteractive(
  async (I) => {
    const result = await interactive(`
...
Scenario('On the all-projects page, the recent submissions table is present if the user has create or update permission on some project', ifInteractive(
  async (I) => {
    const result = await interactive(`
        1. Login as the user with access to some, but not all projects
        2. Navigate to the all-projects page
...
Scenario('On the all-projects page, the recent submissions table is not present if the user has no create or update permission', ifInteractive(
  async (I) => {
    const result = await interactive(`
        1. Login as the user with access to some, but not all projects
        2. Navigate to the all-projects page
...
Scenario('On a project page - the submission tools are present if the user has submit permission', ifInteractive(
  async (I) => {
    const result = await interactive(`
...
Scenario('On a project page - the submission tools are not present if the user does not have submit permission', ifInteractive(
  async (I) => {
    const result = await interactive(`
        1. Login as the user with access to the project
        2. Navigate to the project page
...
Scenario('On the node browser - delete buttons are present if the user has delete permission on the project', ifInteractive(
  async (I) => {
    const result = await interactive(`
        1. Login as the user with access to the project
...
Scenario('On the node browser - delete buttons are not present if the user does not have delete permission on the project', ifInteractive(
  async (I) => {
    const result = await interactive(`
        1. Login as the user with access to the project
...
Scenario('The workspace button is present if the user has access', ifInteractive(
  async (I) => {
    const result = await interactive(`
...
Scenario('The workspace button is not present if the user does not have access', ifInteractive(
  async (I) => {
    const result = await interactive(`
        1. Login as the user with workspace access
