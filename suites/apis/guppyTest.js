...
Scenario('I want to make a filtering query without worrying about paginating the results, or whether the result will be > 10k records.', async (I, guppy) => {
  let token = await I.grabCookie('access_token');
  ...
  await guppy.complete.checkQueryResponseEquals(guppy.props.endpoints.downloadEndpoint, queryFile, expectedResponseFile, token.value, queryType);
});