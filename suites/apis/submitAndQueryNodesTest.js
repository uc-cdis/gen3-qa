...
Scenario('submit node without parent', async (sheepdog, peregrine, nodes) => {
  ...
  const parent_res = await peregrine.do.queryNodeFields(nodes.firstNode);
...
Scenario('query on invalid field', async (peregrine, nodes) => {
  const invalid_field = 'abcdefg';
  const node_type = nodes.firstNode.data.type;
  const q = `{
    ${node_type} {
      ${invalid_field}
    }
...
Scenario('filter query by string attribute', async function (sheepdog, peregrine, nodes)  {
  await sheepdog.complete.addNodes(nodes.toFileAsList);

  const test_field = nodes.firstNode.getFieldOfType('string');
  const q = `{
...
Scenario('filter query by boolean attribute', async (commons, peregrine) => {
  ...
  const boolean_state = commons.project.releasable;
  const q = `
  {
...
Scenario('test with_path_to - first to last node', async (peregrine, sheepdog, nodes) => {
  ...
  const res = await peregrine.do.queryWithPathTo(nodes.firstNode, nodes.lastNode);
  peregrine.ask.hasFieldCount(res, nodes.firstNode.name, 1);
...
Scenario('test with_path_to - last to first node', async (peregrine, sheepdog, nodes) => {
  ...
  // TODO: remove try/catch once bug is fixed
  try {
    const res = await peregrine.do.queryWithPathTo(nodes.lastNode, nodes.firstNode);
