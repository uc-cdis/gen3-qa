...
Scenario('Test Google Data Access user2 (signed urls and temp creds) @reqGoogle @googleDataAccess',
  async (I, fence, users, google, files) => {
    ...
    let timestampCreds2KeyFile = bash.runCommand(`stat ${pathToCreds2KeyFile}`);
    ...
    // creds we just saved (based on the user's permissions)
    // attempt to access a file in the bucket
    timestampCreds2KeyFile = bash.runCommand(`stat ${pathToCreds2KeyFile}`);
    console.log(`timestampCreds2KeyFile: ${timestampCreds2KeyFile}`);
...
Scenario('Test Again: Google Data Access user0 (signed urls and temp creds) @reqGoogle @googleDataAccess',
  async (I, fence, users, google, files) => {
    ...
    // creds we just saved (based on the user's permissions)
    // attempt to access a file in the bucket
    let timestampCreds0KeyFile = bash.runCommand(`stat ${pathToCreds0KeyFile}`);
    ...
    // creds we just saved (based on the user's permissions)
    // attempt to access a file in the bucket
    timestampCreds0KeyFile = bash.runCommand(`stat ${pathToCreds0KeyFile}`);
    console.log(`timestampCreds0KeyFile: ${timestampCreds0KeyFile}`);
    ...
    chai.expect(user0AccessQA1Res,
      'First sync: Check User0 CAN access bucket for project: QA.').to.have.property('id');
    ...
    chai.expect(User0signedUrlQA1ResFileContents.data,
      'First sync: Check that User0 could NOT get a signed URL to read file in QA.').to.equal(fence.props.googleBucketInfo.QA.fileContents);
    ...
    chai.expect(user0AccessQA2Res,
      '2nd sync: Check User2 CAN NOT access bucket for project: QA.').to.have.property('status', 403);
    chai.expect(user0AccessTest2Res,
      '2nd sync: Check User2 CAN NOT access bucket for project: test.').to.have.property('status', 403);
    ...
    chai.expect(User0signedUrlQA2Res,
      '2nd sync: Check that User0 could NOT get a signed URL to read file in QA.').to.have.property('status', 401);

    chai.expect(User0signedUrlTest2Res,
      '2nd sync: Check that User0 could NOT get a signed URL to read file in test.').to.have.property('status', 401);

    // CLEANUP
    console.log('Finally: Ensure cleanup as successful');

    chai.expect(deleteCreds0Res,
      'Cleanup of temporary Google creds for User 0').to.have.property('status', 204);

    chai.expect(deleteServiceAccount0Res,
      'Cleanup of Google service account for User 0').to.be.empty;
  }
);

Scenario('Test Again: Google Data Access user1 (signed urls and temp creds) @reqGoogle @googleDataAccess',
  async (I, fence, users, google, files) => {
    ...
    console.log(`tempCreds0Res: ${JSON.stringify(tempCreds1Res)}`);
    ...
    // creds we just saved (based on the user's permissions)
    // attempt to access a file in the bucket
    let timestampCreds1KeyFile = bash.runCommand(`stat ${pathToCreds1KeyFile}`);
    ...
    // creds we just saved (based on the user's permissions)
    // attempt to access a file in the bucket
    timestampCreds1KeyFile = bash.runCommand(`stat ${pathToCreds1KeyFile}`);
    console.log(`timestampCreds1KeyFile: ${timestampCreds1KeyFile}`);
    ...
    chai.expect(user0AccessQA1Res,
      'First sync: Check User1 CAN access bucket for project: QA.').to.have.property('id');
    chai.expect(user0AccessTest1Res,
      'First sync: Check User1 CAN access bucket for project: test.').to.have.property('id');
    ...
    chai.expect(User1signedUrlQA1ResFileContents.data,
      'First sync: Check that User1 could get a signed URL to read file in QA.').to.equal(fence.props.googleBucketInfo.QA.fileContents);

    chai.expect(User1signedUrlTest1ResFileContents.data,
      'First sync: Check that User1 could get a signed URL to read file in test.').to.equal(fence.props.googleBucketInfo.test.fileContents);
    ...
    chai.expect(user1AccessQA2Res,
      '2nd sync: Check User1 CAN NOT access bucket for project: QA.').to.have.property('status', 403);
    chai.expect(user1AccessTest2Res,
      '2nd sync: Check User1 CAN access bucket for project: test.').to.have.property('id');
    ...
    chai.expect(User1signedUrlQA2Res,
      '2nd sync: Check that User1 could NOT get a signed URL to read file in QA.').to.have.property('status', 401);

    chai.expect(User1signedUrlTest2ResFileContents.data,
      '2nd sync: Check that User1 could get a signed URL to read file in test.').to.equal(fence.props.googleBucketInfo.test.fileContents);

    // CLEANUP
    console.log('Finally: Ensure cleanup as successful');

    chai.expect(deleteCreds1Res,
      'Cleanup of temporary Google creds for User 1').to.have.property('status', 204);

    chai.expect(deleteServiceAccount1Res,
      'Cleanup of Google service account for User 1').to.be.empty;
  }
);