...
async function waitForAuditLogs(category, userTokenHeader, params, nLogs) {
     ...
     * false otherwise
     */
    const areLogsThere = async function(category, userTokenHeader, params, nLogs) {
        // query audit logs
        const json = await module.exports.query(category, userTokenHeader, params);
        if (json.data.length == nLogs) {
            return true;
        } else if (json.data.length > nLogs) {
            // this should never happen
            console.error(`Expected to receive ${nLogs} audit logs or less, bugt received ${json.data.length}.`);
        }
        console.log(`Expecting ${nLogs} logs, received ${json.data.length} logs - waiting...`);
        return false;
    };
    ...
    await smartWait(areLogsThere, [category, userTokenHeader, params, nLogs], timeout, errorMessage, startWait);
};
...
module.exports = {
  ...
  async checkQueryResults(logCategory, userTokenHeader, params = [], nExpectedLogs, expectedResults) {
    /**
    ...
    expectedResults.forEach(function (expectedResult) {
        ...
        const found = receivedLogs.some(function(receivedLog) {
            // return true if all the fields in this received log match
            // the current expected log, false otherwise
            return Object.entries(expectedResult).every(([field, expectedValue]) => {
                return receivedLog[field] == expectedValue;
            });
        });
