...
module.exports = {
  ...
  async generateAndAddRecord(sheepdog, nodeName) {
  ...
  async submitGraphAndFileMetadata(sheepdog, fileGuid = null, fileSize = null, fileMd5 = null, submitter_id = null, consent_codes = null) {
    ...
    // add a link to a `core_metadata_collection` record if needed
    if (!metadata.data.hasOwnProperty("core_metadata_collections") || !metadata.data.core_metadata_collections.hasOwnProperty('submitter_id')) {
      console.log("Did not find core_metadata_collections link in data_file node. Adding one.")
      const cmcSubmitterID = await module.exports.generateAndAddRecord(sheepdog, 'core_metadata_collection');
