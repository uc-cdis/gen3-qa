...
const ENV_TYPE = { JENKINS: 0, LOCAL_AGAISNT_GEN3_K8S: 1, LOCAL_AGAINST_DOCKER: 2 };
...
function getEnvType(){
  ...
  else if (process.env.RUNNING_LOCAL !== undefined && process.env.RUNNING_LOCAL === 'true') {
    if (process.env.NAMESPACE !== undefined && process.env.NAMESPACE !== '')
    {
      console.error('Running against k8s cluster.')
      return ENV_TYPE.LOCAL_AGAISNT_GEN3_K8S;
    }
    else
...
class Env{
  ...
  static setupEnvVariables() {
    ...
    if (envVal === ENV_TYPE.JENKINS || envVal === ENV_TYPE.LOCAL_AGAISNT_GEN3_K8S)
    {
      process.env.HOSTNAME = `${Env.getSubDomain()}.planx-pla.net`;
