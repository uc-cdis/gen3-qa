...
if [[ "$(hostname)" == *"cdis-github-org"* ]] || [[ "$(hostname)" == *"planx-ci-pipeline"* ]]; then
  ...
  npx selenium-standalone install --drivers.chrome.version=96.0.4664.45
  timeout $seleniumTimeout npx selenium-standalone start --drivers.chrome.version=96.0.4664.45 &> selenium.log &
