...
docker run -d -v "${TEST_DATA_PATH}:/mnt/data" --rm --name=dsim --entrypoint=data-simulator quay.io/cdis/data-simulator:master submission_order --url "${TEST_DICTIONARY}" --path /mnt/data --node_name $leafNode
...
docker run -d -v "${TEST_DATA_PATH}:/mnt/data" -v "$(pwd):/tmp/" --rm --name=dsim --entrypoint=data-simulator quay.io/cdis/data-simulator:master submitting_data --host https://${namespace}.planx-pla.net  --dir /mnt/data --project jnkns/jenkins --access_token /tmp/token
