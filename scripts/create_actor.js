...
let { getFiles } = require('./actor_templates.js');
...
rl.question('Select an option:\nCreate an API actor [1] or Portal actor [2]: ', (answer) => {
  rl.question('Type in the actor name: ', (actor_name) => {
    let all_files = getFiles(actor_name);
    let actor_type_dir = answer === 1 ? './actors/apis' : './actors/portal';
...
function createActor(actor_name, type_dir, all_files) {
  ...
  let actor_dir = `${type_dir}/${actor_name}`;

  if (!fs.existsSync(type_dir)) {
    throw new Error(`Unable to find ${type_dir}, make sure you are running the command from the root directory of the project.`)
  ...
  Object.values(all_files).forEach(this_file => {
    fs.writeFile(`${actor_dir}/${this_file.name}`, this_file.template, function (err) {
      if (err) {
        console.log(err);
